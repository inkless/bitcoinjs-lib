...
const getScriptSize = require('./getScriptSize').getScriptSize
...
describe('bitcoinjs-lib (transactions)', function () {
  ...
  it('can create (and broadcast via 3PBP) a Transaction, w/ a P2SH(P2MS(2 of 4)) (multisig) input', async function () {
    const wifs = [
      'cUYHBvPWCdX6oCXrWjrz2ddZtxp6WzvcehMmqh5oxB4ZvCGmiaCQ',
      'cTp6g9vfeyUSwVz8s4Lugcg1A2F4L77gcZSsodoKtHzLivb8T9wX',
      'cThvBJBYTQEWPcoZcp5LXmgmk32kaoyJ7teRxYr2aUvWYdbwzt9C',
      'cRYakUKCYQCXSWA9KCVDLsg4Zpp4ku3wKQX1Qycv52zVgncaXgN6',
    ]

    const keyPairs = wifs.map(wif => {
      return bitcoin.ECPair.fromWIF(wif, bitcoin.networks.testnet)
    })

    const testnet = bitcoin.networks.testnet
    const pubkeys = keyPairs.map(x => x.publicKey)
    console.log('pubkeys', pubkeys.map(pk => pk.toString('hex')))
    const addresses = keyPairs.map(x => {
      return bitcoin.payments.p2pkh({ pubkey: x.publicKey, network: bitcoin.networks.testnet }).address
    })
    ...
    console.log('wallet address', p2sh.address)

    const feeRate = 250 // satoshis per byte
    const resp = await fetch(`https://testnet.blockexplorer.com/api/addr/${p2sh.address}/utxo/?noCache=1`)
    const data = await resp.json()
    const scriptSize = getScriptSize(2, p2sh.redeem.output.length)
    const utxos = data.map(utxo => ({
      txId: utxo.txid,
      value: utxo.satoshis,
      vout: utxo.vout,
      script: Array(scriptSize + 1).join('a'),
    }))
    const targets = [{
      address: 'mrw6PrNMJHRp1tMdnK49zvyHhEMBkqHmBc',
      value: 1e4,
    }]
    ...
    outputs.forEach(output => {
      ...
      if (!output.address) {
        ...
        // use same wallet for now
        output.address = p2sh.address
      }
    ...
    const newTxb = bitcoin.TransactionBuilder.fromTransaction(bitcoin.Transaction.fromHex(hexTx), testnet)
    newTxb.sign(0, keyPairs[0], p2sh.redeem.output)
    const newTx = newTxb.build()
    console.log(newTx.toHex())

//     dhttp({
//       method: 'GET',
//       url: `https://testnet.blockexplorer.com/api/addr/${p2sh.address}/utxo/?noCache=1`
//     }, function (err, body) {
// //       const unspent = body[body.length - 1]
